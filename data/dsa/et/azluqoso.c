...
void dijkstra(struct node *graph[], int n, int b)
{
	for (int i = 0; i < n; ++i)
	{
		graph[i]->c='r';graph[i]->priority=0; graph[i]->start=graph[i]->current;
	}
	int m=0;
	struct node *a[n];
	for (int i = 0; i < n; ++i)
...
int dijkstra_modified(struct node *graph[], int n, int b)
{
	for (int i = 0; i < n; ++i)
	{
		graph[i]->c='r';graph[i]->priority=0; graph[i]->start=graph[i]->current;
...
int shortest_path_modified(struct node * graph[], int n, int a, int b)
{	
	dijkstra_modified(graph,n,a);
	struct node * v = (struct node*)malloc(sizeof(struct node));
	for (int i = 0; i < n; ++i)
	{
		if (graph[i]->value==b)
...
int main()
{
	...
	for (int j = 0; j < test; ++j)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			graph[i]->c='r';
			graph[i]->value=i+1;
			graph[i]->check=graph[i]->priority=0;
		}
		int s,t;
		scanf("%d",&s);
		scanf("%d",&t);
		...
		int d,d1,d2;
		d=shortest_path(graph,n,1,n);
		for (int i = 0; i < n; ++i)
		{
			d1=shortest_path(graph,n,1,graph[i]->value);
			d2=shortest_path(graph,n,graph[i]->value,n);
			if (d1+d2==d)
			{
				graph[i]->check=1;
			}
		}
		int path=shortest_path_modified(graph,n,s,t);
		if (path!=0)
		{
			printf("%d\n",path);
		}
